...
console.log(
    `Child Global Pos (Before Rotate): (${child.transform.globalPosition.x}, ${child.transform.globalPosition.y}) (Expected: 150, 100)`
);
...
console.log(
    `Child Global Pos (After Rotate): (${Math.round(childGlobal.x)}, ${Math.round(childGlobal.y)}) (Expected: 100, 150)`
);
...
console.log(
    `Random UUID: ${import('../src/utility/uuid.js').then(m => console.log(m.UUID.generate()))}`
);
